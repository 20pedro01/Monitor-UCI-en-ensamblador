...
.data
    ...
    ; Estructuras de Captura de Datos (Buffers estructurados para interrupción 21h ah=0Ah)
    paciente_id_buf     db 30, 0, 30 dup(0)
    nombre_paciente_buf db 60, 0, 60 dup(0)

    paciente_id         db 30 dup(' ')
    paciente_id_len     dw 0
    nombre_paciente     db 60 dup(' ')
    nombre_paciente_len dw 0
    ...
    ritmo_cardiaco      dw 0                     ; Dato 1: Frecuencia Cardíaca (FC)
    presion_sistolica   dw 0                     ; Dato 2: Presión Sistólica (SIS)
    presion_diastolica  dw 0                     ; Dato 3: Presión Diastólica (DIA)
    oxigenacion         dw 0                     ; Dato 4: Oxigenación (SpO2)
    temperatura         dw 0                     ; Dato 5: Temperatura Celsius (TEMP_C)
    pam                 dw 0                     ; Dato 6: Presión Arterial Media
    temp_f              dw 0                     ; Dato 7: Temperatura Fahrenheit (TEMP_F)
    estado_salud        dw 0                     ; Dato 8: Estado clínico (0=Normal, 1=Alerta, 2=Critico)
    ...
    msg_titulo          db "=============================================$"
    msg_titulo2         db "    MONITOR DE PACIENTE UCI - SISTEMA V1.0   $"
    msg_titulo3         db "=============================================$"
    
    msg_id              db "Ingrese No. Expediente: $"
    msg_nombre          db "Ingrese Nombre del paciente: $"
    msg_edad            db "Ingrese Edad del paciente: $"
    msg_genero          db "Ingrese Genero del paciente (M/F): $"
    msg_peso            db "Ingrese Peso del paciente (kg): $"
    msg_estatura        db "Ingrese Estatura del paciente (cm): $"
    msg_cama            db "Ingrese Numero de Cama: $"
    msg_medico          db "Ingrese Medico Responsable: $"
    msg_err_vacio       db "Error: Campo obligatorio. Intente de nuevo.$"
    msg_err_numero      db "Error: Debe ingresar solo numeros enteros.$"
    msg_err_genero      db "Error: Debe ingresar M (Masculino) o F (Femenino).$"
    msg_err_decimal     db "Error: Debe ingresar un numero valido (entero o decimal).$"
    msg_iniciando       db "Iniciando captura y lectura de sensores...$"
    
    msg_paciente        db "NO. EXPEDIENTE: $"
    msg_nombre_lbl      db "NOMBRE: $"
    msg_lbl_edad        db " | EDAD: $"
    msg_lbl_genero      db " | GENERO: $"
    msg_lbl_peso        db "PESO: $"
    msg_lbl_estatura    db " kg | ESTATURA: $"
    msg_unit_estatura   db " cm$"
    msg_lbl_cama        db " | CAMA: $"
    msg_lbl_medico      db "MEDICO: $"
    msg_sec_datos       db "Signos Vitales del Paciente:$"
    msg_lbl_fc          db "  - Frecuencia Cardiaca:  $"
    msg_unit_fc         db " bpm$"
    msg_lbl_presion     db "  - Presion Arterial:     $"
    msg_slash           db " / $"
    msg_unit_presion    db " mmHg$"
    msg_lbl_pam         db "  - Presion Media (PAM):  $"
    msg_lbl_spo2        db "  - Oxigeno (SpO2):       $"
    msg_unit_spo2       db " %$"
    msg_lbl_temp        db "  - Temperatura Corporal: $"
    msg_unit_temp       db " C ($"
    msg_unit_temp_f     db " F)$"
    
    msg_lbl_estado      db "---------------------------------------------$"
    msg_lbl_estado2     db "ESTADO CLINICO: $"
    
    msg_txt_normal      db "NORMAL (Estable)$"
    msg_txt_alerta      db "ALERTA (Bajo observacion)$"
    msg_txt_critico     db "CRITICO (Emergencia de UCI - Alarma activada)$"
    
    msg_separador       db "=============================================$"
    msg_err_archivo     db "Error en la apertura o lectura de archivos.$"
    msg_err_lecturas    db "Error: No se pudo abrir lecturas.txt$"
    msg_err_bitacora    db "Error: No se pudo crear o abrir bitacora.txt$"
    msg_fin_sim         db "Fin de la simulacion de lecturas de sensores.$"
    
    ; Variables para el historial (Manejo de archivos: leer y modificar el mismo archivo)
    num_registros       dw 0
    msg_historial_1     db "Historial: Se detectaron $"
    msg_historial_2     db " registros previos en bitacora.txt.$"

    ; Buffers para formatear números en memoria
    str_num3            db "   $"                     ; 3 caracteres para pantalla
    str_num2            db "  $"                      ; 2 caracteres para pantalla
    newline             db 13, 10, "$"
    heart_state         db 0
    space_str           db "  $"
    ...
    msg_tx              db " [TX-COM1]: Transmitiendo trama de telemetria...$"
    msg_rx              db " [RX-COM1]: Comando central recibido -> [ACK: OK]$"
...
main PROC
    ; Inicializar segmentos de datos en 16 bits
    mov ax, @data
    mov ds, ax
    mov es, ax
...
salir:
    mov ax, 4C00h       ; Terminar ejecución en DOS de 16 bits
    int 21h
main ENDP

; =============================================================================
; 4. PROCEDIMIENTOS DE PEDRO (mon16.asm)
; =============================================================================
...
; Entrada: SI = Dirección del búfer estructurado DOS
; Salida:  AL = 0 (Válido), AL = 1 (Vacío)
ValidarVacio PROC
    push bx
    xor ax, ax
    mov al, [si+1]      ; Obtener longitud real ingresada
    or al, al
    jz es_vacio
...
ValidarNumerico PROC
    ...
    xor ax, ax
    mov cl, [si+1]      ; CL = longitud ingresada
    or cl, cl
    jz num_invalido     ; Si está vacío, es inválido
    
    add si, 2           ; SI apunta al inicio de los caracteres
chk_num_loop:
    mov al, [si]
    cmp al, '0'
    jl num_invalido
...
ValidarGenero PROC
    push bx
    xor ax, ax
    mov al, [si+1]      ; Obtener longitud
    cmp al, 1
    jne gen_invalido    ; Debe ser de longitud 1
    
    mov al, [si+2]      ; Obtener el carácter ingresado
    cmp al, 'M'
    je gen_valido
    cmp al, 'm'
    je gen_valido
...
ValidarDecimal PROC
    ...
    xor bx, bx          ; BH = contador de puntos (0 o 1), BL = índice actual (0-based)
    xor dx, dx          ; DH = contador de decimales
    add si, 2           ; SI apunta al inicio de los caracteres
